...
<app:url var="baseUrl"/>
<app:url var="searchUrl" value="/locate/item/filter"/>
<app:url var="sortUrl" value="/locate/item/filter/sortby"/>

<c:set var="doReturnSelected" value="doLayerReturnSelected( window.parent, 'locateItemFilterResultId', 'itemLayer');"/>

<div class="search locate">

    <form:form modelAttribute="locateItemFilter" id="locateItemFilterId" method="POST" focus="filterValue" action="${searchUrl}" >
    ...
    <c:if test="${locateItemFilterResult.result != null}">
        ...
        <table class="searchResult" width="95%" >
            <tr>
                <td class="resultCount label">
                    ...
                    <label class="value"></label>${fn:length(locateItemFilterResult.result)}
                </td>
    ...
    <form:form id="locateItemFilterResultId" modelAttribute="locateItemFilterResult" method="POST">
        <c:if test="${locateItemFilterResult.result != null}">
            ...
            <table class="searchResult" width="95%">
                ...
                <tr class="row">
					...
					<th class="cell cell-text cell-element">
                        <a href="javascript:checkAll('locateItemFilterResultId', 'selectedItems.selectableObjects', true)"><app:message code="admin.global.filter.label.selectAll" /></a><br>
                        <a href="javascript:checkAll('locateItemFilterResultId', 'selectedItems.selectableObjects', false)"><app:message code="admin.global.filter.label.clearAll" /></a>
                    </th>
                </tr>
                ...
                <c:forEach var="item" items="${locateItemFilterResult.result}" varStatus="i">
                    <tr class="row">
                        <td class="cell cell-number"><c:out value="${item.value.itemId}"/></td>
                        <td class="cell cell-number"><c:out value="${item.value.itemSku}"/></td>
                        <td class="cell cell-text"><a href="javascript:void(0);" onclick="checkOneOfAll('locateItemFilterResultId', 'selectedItems.selectableObjects','item_${item.value.itemId}');return ${doReturnSelected}"><c:out value="${item.value.itemName}"/></a></td>
