...
public class AmountEmptyAllowedValidator implements CustomValidator<String, ValidationCodeResolver> {
    private BigDecimal range;
    private BigDecimal parsedValue;
    
    public AmountEmptyAllowedValidator () {
        range = null;
    }
    
    public AmountEmptyAllowedValidator (Integer precision, Integer scale) {
        if (precision != null &&
            precision > 0 &&
            scale != null &&
            scale >= 0 &&
    ...
    private void setParsedValue(BigDecimal parsedValue) {
